...
def transform_custom(data, *args, **kwargs):
    ...
    target=None,
    numerical_features=x_sensors_colnames,
    categorical_features=None
    )

    report = Report(metrics=
        [DatasetDriftMetric()] #+ [ColumnDriftMetric(column_name=sensor) for sensor in x_sensors_colnames],

    )

    report.run(reference_data=reference_data[-700:], current_data=current_data, column_mapping=column_mapping)

    drift_share = report.as_dict()['metrics'][0]['result']['drift_share']
    
    return complete_current_data[["datetime"] + x_sensors_colnames + ["sid_35606"]], drift_share
